...
const RecruiterDashboard = () => {
    ...
    useEffect(() => {
        const fetchJobs = async () => {
            try {
                ...
                // ✅ FIX: Added Cache-Control Headers and Cache-Buster timestamp
                const config = { 
                    headers: { 
                        'x-auth-token': token,
                ...
                // The ?t=Date.now() forces the browser to pull fresh data from the Database
                const res = await API.get(`/my-posted-jobs?t=${Date.now()}`, config);
                setJobs(res.data);
            } catch (err) {
                                ...
                                {filteredJobs.length === 0 ? (
                                    ...
                                    filteredJobs.map((job) => (
                                        <tr key={job._id} className="hover:bg-slate-50/80 transition group even:bg-slate-50/30">
                                            ...
                                            {/* ✅ SALARY COLUMN WILL NOW UPDATE INSTANTLY */}
    ...
    <div className="flex justify-center gap-2">
        ...
        <button 
            onClick={() => navigate(`/manage-job/${job._id}`)} 
            className="p-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-all duration-200 group" 
        ...
        <button 
            ...
            className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all duration-200 group" 
            title="Edit Job"
        >
            <svg className="w-5 h-5 group-hover:scale-110 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor">
